...
$totalItemsQuery = "SELECT COUNT(*) as total FROM return_emergency";
$totalItemsResult = $conn->query($totalItemsQuery);
$totalItems = $totalItemsResult->fetch_assoc()['total'];
$totalPages = ceil($totalItems / $itemsPerPage);

$projects = $conn->query("SELECT DISTINCT project_name FROM return_emergency WHERE status = 0 LIMIT $offset, $itemsPerPage");
?>
...
<head>
    ...
    <title>Borrow Maintenance</title>
</head>
<style>
    .header-container {
        padding-bottom: 5px;
...
<body>
    ...
    <main>
        ...
        if ($projects->num_rows > 0) {
            while ($project = $projects->fetch_assoc()) {
                ...
                $sql = "SELECT * FROM return_emergency WHERE project_name = '$projectName' AND status = 0";
    ...
    <div id="approve-confirmation-overlay" class="approve-confirmation-overlay">
        <div class="approve-confirmation-content">
            <h3>Are you sure to maintenance the request?</h3>
    ...
    <script>
        ...
        function submitApproveItems(itemIds) {
            fetch('approve_maintenance_request.php', {
                method: 'POST',
        ...
        function submitApproveItem() {
            ...
            document.querySelectorAll('.project-container').forEach(container => {
                container.querySelectorAll('.item input[type="checkbox"]:checked').forEach(checkbox => {
                    selectedIds.push(checkbox.closest('.item').getAttribute('data-id'));
                });
            });
