...
<body>
    ...
    <main>
        <div class="header-container">
            <h1>Emergency Maintenance Report</h1>
        ...
        <div class="filter-print-container">
            ...
            <form class="filter-form" method="GET" action="">
                ...
                <select id="year" name="year">
                    ...
                    <?php
                    $years = $conn->query("SELECT DISTINCT YEAR(borrowed_date) AS year FROM return_emergency ORDER BY year DESC");
                ...
                <select id="borrowed_month" name="borrowed_month">
                    <option value="">All</option>
                    <?php
                    // Generate month options for borrowed_date
                    for ($m = 1; $m <= 12; $m++) {
                ...
                <select id="return_month" name="return_month">
                    <option value="">All</option>
                    <?php
                    // Generate month options for return_date
                    for ($m = 1; $m <= 12; $m++) {
                        $monthName = date("F", mktime(0, 0, 0, $m, 10));
                        $monthValue = str_pad($m, 2, "0", STR_PAD_LEFT);
        ...
        <div class="inventory-container">
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM return_emergency WHERE 1=1";
                    
                    // Add filters to the SQL query
                    if ($year) {
                        $sql .= " AND YEAR(borrowed_date) = '$year'";
                    }
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $row["status"] == 0 ? 'Requested' : ($row["status"] == 1 ? 'Maintenance' : 'Finished'); ?></td>
                            </tr>
                            <?php
                        }
                    } else {
    ...
    <script>
        function printContent() {
            ...
            printWindow.document.write(document.querySelector('main').innerHTML);
